...
// Append select dropdowns to form
...
pokemons.forEach((e) => {
  ...
  h2.textContent = e.type;
  h2.style.backgroundColor = "#C4E4FF";
  h2.style.borderRadius = "10px";

  let h5 = document.createElement("h5");
  h5.textContent = `Candy count: ${e.candy_count}`;
  h5.style.backgroundColor = "yellow";

  let h3 = document.createElement("h3");
  h3.textContent = e.weight;
  h3.style.backgroundColor = "yellow";

  let p = document.createElement("p");
  p.textContent = e.weaknesses.join(", ");
  p.style.backgroundColor = "yellow";
  p.style.color = "red";

  let p2 = document.createElement("p");
  p2.textContent = e.num;
  ...
  card.append(p2, h1, img, h2, h5, h3, p, p3);

  cards.appendChild(card);
});

// Populate select dropdown for type
let uniqueTypes = new Set();
pokemons.forEach((e) => uniqueTypes.add(e.type));
...
// Populate select2 dropdown for weight
let uniqueWeights = new Set();
pokemons.forEach((e) => uniqueWeights.add(e.weight));
...
input.addEventListener("input", () => {
  ...
  document.querySelectorAll(".card").forEach((card) => {
    ...
    let type = card.querySelector("h2").textContent.toLowerCase();
    let weight = card.querySelector("h3").textContent.toLowerCase();
    let num = card.querySelector("p:nth-child(1)").textContent.toLowerCase();
    let weaknesses = card.querySelector("p:nth-child(7)").textContent.toLowerCase();

    if (
      name.includes(inputValue) ||
      type.includes(inputValue) ||
      weight.includes(inputValue) ||
      num.includes(inputValue) ||
      weaknesses.includes(inputValue)
    ) {
      card.style.display = "flex";
    } else {
...
select.addEventListener("change", () => {
  let selectedCategory = select.value.toLowerCase();
  document.querySelectorAll(".card").forEach((card) => {
    let type = card.querySelector("h2").textContent.toLowerCase();
    if (type.includes(selectedCategory)) {
      card.style.display = "flex";
    } else {
      card.style.display = "none";
    }
...
select2.addEventListener("change", () => {
  let selectedWeight = select2.value.toLowerCase();
  document.querySelectorAll(".card").forEach((card) => {
    let weight = card.querySelector("h3").textContent.toLowerCase();
    if (weight.includes(selectedWeight)) {
      card.style.display = "flex";
    } else {
      card.style.display = "none";
    }
